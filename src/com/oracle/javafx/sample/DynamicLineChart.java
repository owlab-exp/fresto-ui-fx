...
public class DynamicLineChart extends Application {
	...
	public void init() throws Exception {
		...
		events = FXCollections
				.observableArrayList(
						new Event("Javelin", "6 6", FXCollections
								.observableArrayList(
										createSeries("Javelin - Tokyo",
												FXCollections
														.observableArrayList(
																18, 20, 22)),
										createSeries("Javelin - Kyoto",
												FXCollections
														.observableArrayList(
																23, 14, 15)))),
						new Event("Hammer", "12 2 2 12", FXCollections
								.observableArrayList(
										createSeries("Hammer - Tokyo",
												FXCollections
														.observableArrayList(
																12, 11, 5)),
										createSeries("Hammer - Kyoto",
												FXCollections
														.observableArrayList(9,
																8, 13)))),
						new Event("Shotput", "", FXCollections
								.observableArrayList(
										createSeries("Shotput - Tokyo",
												FXCollections
														.observableArrayList(3,
																2, 4)),
										createSeries("Shotput - Kyoto",
												FXCollections
														.observableArrayList(4,
																6, 5)))));
		...
		final TitledPane controlPane = new TitledPane("Event Selection",
				eventChecks);
		...
		for (final Event event : events) {
			...
			StringBuilder styleString = new StringBuilder(
					"-fx-stroke-width: 3; -fx-stroke: gray;");
			if (event.getStrokeDashArray() != null
					&& !event.getStrokeDashArray().isEmpty()) {
				styleString.append("-fx-stroke-dash-array: ")
						.append(event.getStrokeDashArray()).append(";");
			}
			...
			box.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent action) {
					...
					// the Timeline is a small hack to style series again a
					// little while after the
					// checkbox was pressed to allow the initial layout and css
					// styling pass for
					// the newly active series to have already passed so that a
					// subsequent lookup
					// by css ids works.
		...
		// layout the scene
		VBox captionedChart = VBoxBuilder
				.create()
				.spacing(10)
				.alignment(Pos.CENTER)
				.children(
						HBoxBuilder.create().spacing(10).alignment(Pos.CENTER)
								.children(controlPane, lineChart).build(),
						caption).build();
	...
	private void populateData(final ObservableList<Event> events,
	...
	private void styleSeries(ObservableList<Event> events,
			final LineChart<Number, Number> lineChart) {
		// mark different series with different depending on whether they are
		...
		for (int i = 0; i < events.size(); i++) {
			...
			for (int j = 0; j < event.getSeries().size(); j++) {
				XYChart.Series<Number, Number> series = event.getSeries()
						.get(j);
	...
	private class Event {
		...
		public Event(String name, String strokeDashArray,
		...
		public boolean isBelowAverage(XYChart.Series checkedSeries) {
			...
			double allSeriesAvg = seriesCount != 0 ? allSeriesAvgTot
					/ seriesCount : 0;
